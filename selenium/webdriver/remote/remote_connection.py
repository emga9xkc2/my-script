...
class RemoteConnection(object):
    ...
    def _get_connection_manager(self):
        self._timeout = self.TIMEOUT_REQUEST
        pool_manager_init_args = {"timeout": self._timeout}
        if self._ca_certs:
            pool_manager_init_args["cert_reqs"] = "CERT_REQUIRED"
            pool_manager_init_args["ca_certs"] = self._ca_certs

        return urllib3.PoolManager(**pool_manager_init_args) if not self._proxy_url else urllib3.ProxyManager(self._proxy_url, **pool_manager_init_args)

    def __init__(self, remote_server_addr, keep_alive=False, resolve_ip=None, ignore_proxy=False):
        if resolve_ip:
            import warnings

            warnings.warn("'resolve_ip' option removed; ip addresses are now always resolved by urllib3.", DeprecationWarning)
