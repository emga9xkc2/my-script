...
from selenium.common.exceptions import InvalidArgumentException, JavascriptException, WebDriverException, NoSuchCookieException
...
class WebDriver(BaseWebDriver):
    ...
    def __init__(
        ...
        if desired_capabilities:
            warnings.warn("desired_capabilities has been deprecated, please pass in an Options object with options kwarg", DeprecationWarning, stacklevel=2)
        if browser_profile:
            warnings.warn("browser_profile has been deprecated, please pass in an Firefox Options object with options kwarg", DeprecationWarning, stacklevel=2)
        if proxy:
            warnings.warn("proxy has been deprecated, please pass in an Options object with options kwarg", DeprecationWarning, stacklevel=2)
        if not keep_alive:
            warnings.warn("keep_alive has been deprecated. We will be using True as the default value as we start removing it.", DeprecationWarning, stacklevel=2)
        capabilities = {}
        ...
        if isinstance(self.command_executor, (str, bytes)):
            self.command_executor = get_remote_connection(capabilities, command_executor=command_executor, keep_alive=keep_alive, ignore_local_proxy=_ignore_local_proxy)
    ...
    def execute(self, driver_command: str, params: dict = None) -> dict:
        ...
        """
        # if self.hdebug:
        #     print(time.time(), driver_command, params)
        if self.session_id:
            if not params:
