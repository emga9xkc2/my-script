...
class WebElement(BaseWebElement):
    ...
    def submit(self):
        ...
        self._parent.execute_script(
            "var e = arguments[0].ownerDocument.createEvent('Event');" "e.initEvent('submit', true, true);" "if (arguments[0].dispatchEvent(e)) { arguments[0].submit() }", form
        )
    ...
    def get_attribute(self, name) -> str:
        ...
        attribute_value = self.parent.execute_script("return (%s).apply(null, arguments);" % getAttribute_js, self, name)
    ...
    def send(self, *value, timesleep=0.1) -> None:
        for i in value:
            for j in i:
                self.send_keys(j)
                time.sleep(timesleep)

    def send_keyss(self, *value, key=None, timesleep=0.1) -> None:
        self.clear()
        for i in value:
            for j in i:
                self.send_keys(j)
