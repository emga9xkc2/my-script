import re
import download.pytz as pytz
from copy import copy
from datetime import datetime

from download.timestring.Date import Date
from download.timestring import TimestringInvalid
from download.timestring.timestring_re import TIMESTRING_RE
...
class Range(object):
    def __init__(self, start, end=None, offset=None, start_of_week=0, tz=None, verbose=False):
        """`start` can be type <class download.timestring.Date> or <type str>
        """
        self._dates = []
        pgoffset = None
    ...
    def __repr__(self):
        return "<download.timestring.Range %s %s>" % (str(self), id(self))
